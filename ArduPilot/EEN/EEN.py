...
def change_parameter():
    ...
    print("# [Change_parameter()] # of parameters: %d, selected params: %s" % (
    parameter_num, read_meta_parameter.param_name[selected_param]))
    ...
    if range_min == 'X':
        ...
        read_meta_parameter.param_name[selected_param], param_value))
        ...
        if range_min.isdigit() == True and range_max.isdigit() == True:
            ...
            read_meta_parameter.param_name[selected_param], float(range_min), float(range_max), param_value))
    ...
    while True:
        if stable_counter > 0:
            ...
            print(
                ...
                roll_error, pitch_error, heading_error, current_altitude, alt_error, stable_counter,
                current_flight_mode))
            stable_counter = 0
            alt_error = 0
            roll_error = 0
...
print("# Check whether parsing parameters well done or not, received # of params: %d" % len(
    read_meta_parameter.param_name))
