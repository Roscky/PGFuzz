...
# Tell python where to find mavlink so we can import ithttps://github.com/Roscky/PGFuzz.git
# ?打印目录后返现该目录并不存在，/home/pgfuzz/pgfuzz/ArduPilot/Dynamic analysis/../mavlink，但是python库包含pymavlink，可以直接import
sys.path.append(os.path.join(os.path.dirname(os.path.realpath(__file__)), '../mavlink'))
# print 'syspath: ' + os.path.join(os.path.dirname(os.path.realpath(__file__)), '../mavlink')
...
# ------------------------------------------------------------------------------------
# Global variables
# 创建与SITL的连接，SITL输出端口在启动时会显示，连接时使用udpin或者udp（等同udpin）
# setting up a link to (initially) listen for messages or send messages on a channel (e.g. udp, serial, etc.). This returns an object representing the connection
# mav attribute for accessing the selected dialect/protocol module to encode and send messages
...
# -------------------------
# avg_<param_name>：为<param_name>的列表，存储了该参数随时间变化的值，用于计算方差判断该参数是否发生变化
avg_start = 0
avg_airspeed = []
avg_groundspeed = []
avg_heading = []
...
# target_param：当前分析的参数名称
# target_param_ready：当前分析参数是否ready
# target_param_value：当前分析参数的具体值
target_param = ""
...
def handle_position(msg):
    ...
    if avg_start == 1:
        # Store average of hud data
        position_cnt += 1
        # 由于浮点数的分辨率不足，因此它被设计为整数消息,所以这是在干嘛？转浮点数？
        lat = msg.lat / 1000
        lat = lat * 1000
        lon = msg.lon / 1000
        lon = lon * 1000
...
def read_loop():
    while True:
        ...
        if msg_type == "BAD_DATA":
            ...
            # 更新全局变量current_rc_3
            # 如avg_start == 1，则在avg_rc(1-4)列表中添加当前avg_rc1-4的值
            handle_rc_raw(msg)
        elif msg_type == "HEARTBEAT":
            # 更新全局变量current_flight_mode drone_status
            handle_heartbeat(msg)
        elif msg_type == "VFR_HUD":
            # 更新全局变量previous_altitude current_altitude current_aspd
            # 全局变量有VFR_HUD所有数据的列表，如avg_start == 1，列表添加当前数值
            handle_hud(msg)
        elif msg_type == "ATTITUDE":
            # 如avg_start == 1，更新姿势相关数据列表
            handle_attitude(msg)
        elif msg_type == "NAV_CONTROLLER_OUTPUT":
            # 如avg_start == 1，更新导航和位置控制器的状态
            handle_target(msg)
        elif msg_type == "GLOBAL_POSITION_INT":
            # 如avg_start == 1，更新导航和位置控制器的状态
            handle_position(msg)
        elif msg_type == "STATUSTEXT":
            # 检测系统的重要状态（如降落伞释放，GPS错误等）并以此更新全局变量
            handle_status(msg)
            ...
            # 接收参数，如果参数是当前正在分析的参数，则设置param_ready为1，并设置param_value为接收值；否则设置param_ready为0
            handle_param(msg)
...
def set_rc_channel_pwm(id, pwm=1500):
    ...
    if id < 9:
        ...
        # 发送给 MAV 的 RC 信道的 RAW 值覆盖从 RC 无线电收到的信息。标准的 PPM 调制如下: 1000微秒: 0% ，2000微秒: 100% 。单个接收器/发射器可能违反此规范。仔细注意前8个通道和后续通道之间的语义差异
        # A value of UINT16_MAX means to ignore this field.A value of 0 means to release this channel back to the RC radio.
    ...
    for iteration in range(start_profiling, 2):
        ...
        for i in range(itera):
            ...
            for mode_i in range(0, 21):
                ...
                master.mav.set_mode_send(master.target_system, mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED,
                                         mode_i)
                time.sleep(0.2)
                ...
                if targetInput == "RC3" and input_type == "cmd" and iteration != 0:
                    ...
                    print("[Test] %s (%d, %d, %d, %d, %d, %d, %d)" % (
                    targetInput, rand[0], rand[1], rand[2], rand[3], rand[4], rand[5], rand[6]))
                ...
                Baro_status = 1
                PreArm_error = 0
                print ("--------------- [End] measuring states in %s mode (%d)---------------" % (
                current_flight_mode, mode_i))
...
# request data to be sent at the given rate, repeat to make sure get this message
# 先使用master.mav.request_data_stream_send指定无人机发送消息的类型（所有）以及发送速率
# 然后使用master.recv_match
for i in range(0, 3):
...
# Choose a mode
mode = 'GUIDED'
print('Try:', list(master.mode_mapping()))
...
# arm the vehicle，参数1表示是否arm，参数1值为1表示arm，0表示disarm
master.mav.command_long_send(
    master.target_system,
    master.target_component,
    mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM,
...
# 从"cmds.txt"读取可输入的命令和对应的索引号（索引号可以在https://mavlink.io/en/messages/common.html中查看）并打印，所以后面的打印感觉有点重复了，而且顺序还错了
# 从"envs.txt"读取环境参数
...
# 在当前目录下创建result文件夹，并在该文件夹下创建输出文件（以写方式打开，如果之前存在内容会被清空）
init_files()

# 创建3个长度为34的列表，且值都为0，34为状态的数量（34个状态到15个输出参数文件的映射在哪？ -> 在store_result()函数中）
# 34个状态的含义参考：https://github.com/purseclab/PGFUZZ/tree/main/ArduPilot/Dynamic%20analysis
sd = []
...
# 将"preconditions.txt"中的预先设置参数发送给无人机
set_preconditions(Precondition_path)
